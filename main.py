#!/usr/bin/env python
# -*- coding: utf-8 -*-
__appname__ = "ImageDiff"
__copyright__ = "Copyright (c) 2026 Serendea" #Brand Name

__version__ = "1.0" #X.Y=Major Update (UI Change or Additional Function) . Minor Update (Bugfix etc.)
__date__    = "2026/03/29"
__deprecated__ = "Windows 11 64bit 25H2"
...
class Application(tkdnd.TkinterDnD.Tk):
    def __init__(self):
        ...
        global model
        model=Model()
        model.set_poppler_path() # Popplerのパスを通す
        self.icon_image=model.get_abs_path("./imgs/Logo_256x256.ico")
        ...
        self.setting_dir_path = os.environ["LOCALAPPDATA"]+"/ImageDiff".replace("/", os.sep)
        self.setting_file_path = self.setting_dir_path+"/settings.json".replace("/", os.sep)
    ...
    def create_widgets(self):
        ...
        self.main_frame.ctrl_menubar(self.menubar)
...
class Menubar(tk.Menu):
    ...
    def create_file_menu(self):
        ...
        self.file_menu.add_command(label="旧図ファイル選択", command=self.ctrl_frame.old_entry_dialogue)
        self.file_menu.add_command(label="改訂図ファイル選択", command=self.ctrl_frame.new_entry_dialogue)
        self.file_menu.add_command(label="出力先フォルダ選択", command=self.ctrl_frame.outdir_entry_dialogue)
        self.file_menu.add_separator()
        self.file_menu.add_command(label="実行", command=self.ctrl_frame.run_program)
        self.file_menu.add_command(label="中断", command=self.ctrl_frame.stop_program)
    ...
    def create_edit_menu(self):
        ...
        self.edit_menu.add_command(label="選択項目をクリア", command=self.delete_focus)
    ...
    def create_help_menu(self):
        ...
        self.help_menu.add_command(label="使い方", command=self.show_how2use_window)
        self.help_menu.add_command(label="バージョン情報", command=self.show_about_window)
...
class MainFrame(ttk.Frame):
    def __init__(self, master, ctrl_frame):
        ...
        self.thread_flag = threading.Event() # 中断ボタン監視フラグ(set()=true;継続, clear()=false;中断)
        self.create_widgets()

    def ctrl_menubar(self, menubar):
        self.menubar=menubar
        self.file_menu=self.menubar.file_menu
    ...
    def create_widgets(self):
        ...
        self.run_button = ttk.Button(self, text="実行", command=self.run_program)
        self.stop_button = ttk.Button(self, text="中断", command=self.stop_program)
    ...
    def run_program(self):
        self.ctrl_frame.text_message_init() #メッセージボックスを空にする。
        ...
        try:
            ...
            ######################
            file_exts = model.detect_exts(fname1, fname2)
            #エントリーボックスが空
            ...
            thread_main = threading.Thread(target=self.img_comp_flow, args=(fname1, fname2, output_dir))
            ...
            with open(os.path.join(output_dir, "ERR_DETECT_IMAGEDIFF.txt"), "w") as error_output:
    ...
    def img_comp_flow(self, fname1, fname2, output_dir):
        """
        画像の画素差分出力を流す。コアとなる処理はModelクラスから呼び出し。
        """
        file_exts = model.detect_exts(fname1, fname2)
        ######################
        #比較⇒変換⇒出力
            ...
            if self.thread_flag.is_set() == True:
                ...
                if self.thread_flag.is_set() == True:
                    ...
                    for page in range(0, num_frames):
                        ...
                        #新旧青/赤色付け
                        drw_out = model.create_masked_image(
                            drw1[page], drw2[page])
                        #左上文字入れ
                        drw_out = model.insert_text(
...
class ProgressWindowFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.progress_window = tk.Text(self, wrap=tk.WORD, fg="white", bg="black")
...
class How2UseWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        # How2UseWindowのサイズは画像のサイズに合わせる。画像はimgsフォルダ内のHow2Use-1.PNG, How2Use-2.PNG,...の順で表示する。
        imgs_dir = model.get_abs_path("./imgs")
        img_files = sorted([f for f in os.listdir(imgs_dir) if f.startswith("How2Use-") and f.lower().endswith(".png")])
        self.imgs = [Image.open(os.path.join(imgs_dir, f)) for f in img_files]
...
class AboutWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        icon_label = ttk.Label(self, image=self.tk_logo_img, anchor='e')
        separator = ttk.Separator(self, orient=tk.HORIZONTAL)
        ...
        osver_label2 = ttk.Label(self, text=str(__deprecated__), anchor='w')
        ...
        icon_label.grid(row=0, column=0, padx=20, pady=10, columnspan=2)#, sticky="NSEW")
        ...
        osver_label.grid(row=4, column=0, pady=2, sticky="NSEW")
        osver_label2.grid(row=4, column=1, padx=5, pady=2, sticky="NSEW")
        license_label.grid(row=5, column=0, pady=2, sticky="NSEW")
        license_label2.grid(row=5, column=1, padx=5, pady=2, sticky="NSEW")
        close_about_button.grid(row=6, column=0, pady=15, columnspan=2)
        separator.grid(row=7, column=0, columnspan=2, sticky="NSEW")
        copyright_label.grid(row=8, column=0, pady=5, columnspan=2)
...
class Model:
    def get_abs_path(self, relative_path):
        ...
        if relative_path[0]==os.sep:  relative_path=relative_path[1:]
    ...
    def set_poppler_path(self):
        ...
        Popplerのパスを通す
        """
        poppler_path = self.get_abs_path("./poppler-24.08.0/Library/bin")

        # PATH の空要素を除去
        paths = [p for p in os.environ["PATH"].split(os.pathsep) if p.strip()]
        os.environ["PATH"] = os.pathsep.join(paths)
        os.environ["PATH"] = poppler_path + os.pathsep + os.environ["PATH"]

    def detect_exts(self, fname1, fname2):
        """
        fname1=(aaa.pdf, bbb.pdf,), fname2=(ccc.pdf, ddd.pdf,)の場合、
        fname1+fname2=(aaa.pdf, bbb.pdf, ccc.pdf, ddd.pdf)
        fname=aaa.pdf, fname=bbb.pdf, fname=ccc.pdf, fname=ddd.pdfと
        順番に取り出してドットの右半分(拡張子)[1]をリストに代入する。
        file_exts=[.pdf, .pdf, .pdf, .pdf]
        str.lowerはPDFやpdfが混在した場合の区別をなくしている。
        """
        file_exts = [str.lower(os.path.splitext(fname)[1]) for fname in fname1+fname2]
        return file_exts

    def read_binary_image(self, fname1, fname2, file_ext):
        if file_ext.endswith(".tif") or file_ext.endswith(".tiff"):
